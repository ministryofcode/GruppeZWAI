...
CREATE TABLE M_LIKES(ID INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, U_ID NUMERIC FOREIGN KEY REFERENCES M_USER(ID), P_ID NUMERIC FOREIGN KEY REFERENCES M_POSTS(ID), likestatus BOOLEAN);
CREATE TABLE M_MESSAGES(ID INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, S_ID NUMERIC, R_ID NUMERIC, Message VARCHAR(255), Feedback BOOLEAN, READ BOOLEAN)
