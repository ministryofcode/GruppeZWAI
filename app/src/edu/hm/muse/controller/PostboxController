...
public class PostboxController {
	...
	@RequestMapping(value = "/postbox.htm", method = RequestMethod.GET)
	public ModelAndView showPostbox() {
		ModelAndView mv = new ModelAndView("postbox");
		return mv;
	}

	@RequestMapping(value = "/postbox.htm", method = RequestMethod.POST)
	public ModelAndView useFunction(@RequestParam(value = "chooseFunction", required = true) String functionBox,
			HttpSession session) {
		
		System.out.println(functionBox.toString());
		
		ValidatorService vs = new ValidatorService(session.getId());
		if(!vs.isValid())
			return new ModelAndView("loginfalse");
		
		
		if(functionBox.equals("Neue PN")){
			
			return new ModelAndView("newMessage");
			
		}else if (functionBox.equals("Ungelesene Nachrichten")) {
			
			return new ModelAndView("unreadMessages");
			
		}else if (functionBox.equals("Gelesene Nachrichten")){
			
			return new ModelAndView("readMessages");
